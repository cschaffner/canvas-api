...
def assign_same_grade(teams, assignment, dry_run=False, be_nice=False):
    ...
    for team in teams:
        ...
        if team.submissions[0].user.name != 'Jens Dudink':
        ...
        message += "\nIn team quizzes, all students in a team receive the same grade which is equal to the first submission by the team, so {}\n".format(
            team_score)
        if be_nice:
            message += "Because this quiz has been regraded (and the actual submission times lost, thanks Canvas), we take the maximum of all submitted scores for this quiz.\n"
            scores = []
            for subm in team.submissions:
